...
var _ = Describe("Raft", func() {
	Context("peers", func() {
		It("a node should be able to join and leave repeatedly", func() {
			...
			p1 := clusteringtestutil.NewPeer(fake.CharactersN(10), net.ParseIP("127.1.0.1"))
			lp1 := newLocalPeer(p1)
			cancel1, c1, r1, err := newLocalNode(lp1)
			Expect(err).ToNot(HaveOccurred())
			p2 := clusteringtestutil.NewPeer(fake.CharactersN(10), net.ParseIP("127.1.0.2"))
			lp2 := newLocalPeer(p2)
			cancel2, c2, r2, err := newLocalNode(lp2)
			defer cancel2()
			Expect(err).ToNot(HaveOccurred())
			p3 := clusteringtestutil.NewPeer(fake.CharactersN(10), net.ParseIP("127.1.0.3"))
			lp3 := newLocalPeer(p3)
