...
func logfix(std logger) (exported map[string]reflect.Value) {
	...
	exported["SetOutput"] = reflect.ValueOf(func(w io.Writer) {
		errorsx.Log(std.Output(2, fmt.Sprintln("bearded-wookie: changing the default logger is not allowed at this time")))
		panic(1)
	})

	exported["SetPrefix"] = reflect.ValueOf(func(path string) error {
		errorsx.Log(std.Output(2, fmt.Sprintln("bearded-wookie: changing the default log prefix is not allowed at this time")))
		panic(1)
	})

	exported["SetFlags"] = reflect.ValueOf(func(path string) error {
		errorsx.Log(std.Output(2, fmt.Sprintln("bearded-wookie: changing the default log flags is not allowed at this time")))
		panic(1)
	})

	exported["Print"] = reflect.ValueOf(func(v ...interface{}) {
		errorsx.Log(std.Output(2, fmt.Sprint(v...)))
	})

	exported["Printf"] = reflect.ValueOf(func(format string, v ...interface{}) {
		errorsx.Log(std.Output(2, fmt.Sprintf(format, v...)))
	})

	exported["Println"] = reflect.ValueOf(func(v ...interface{}) {
		errorsx.Log(std.Output(2, fmt.Sprintln(v...)))
	})

	exported["Fatal"] = reflect.ValueOf(func(v ...interface{}) {
		msg := fmt.Sprint(v...)
		errorsx.Log(std.Output(2, msg))
		panic(msg)
	})
