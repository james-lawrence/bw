...
func (t *agentCmd) configure(parent *kingpin.CmdClause) {
	...
	(&agentDeploymentCoordinator{agentCmd: t}).configure(parent.Command("coordinator", "run a coordination server that purely acts as a command and control node, you deploy to the cluster and it'll store the archive but not actually execute it."))
