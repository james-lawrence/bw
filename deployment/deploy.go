...
func (t worker) work(ctx context.Context) {
	...
	for op := range t.c {
		...
		deadline, done := context.WithTimeout(ctx, t.timeout)
		if err := op(deadline); err != nil {
			log.Println(err)
			atomic.AddInt64(t.failed, 1)
			errorsx.MaybeLog(agentutil.ReliableDispatch(deadline, t.dispatcher, agentutil.LogError(t.local, err)))
		} else {
			errorsx.MaybeLog(agentutil.ReliableDispatch(deadline, t.dispatcher, agentutil.PeersCompletedEvent(t.local, atomic.AddInt64(t.completed, 1))))
		}
		done()
	}
}
...
func healthcheck(ctx context.Context, task *pending, op operation) (err error) {
	...
	// log.Println("healthcheck", task.Peer.Name, "initiated")
	// defer log.Println("healthcheck", task.Peer.Name, "completed")
	...
	case agent.Deploy_Failed:
		select {
		case task.done <- errors.Errorf("%s: deployment has failed", did):
		case <-ctx.Done():
		}
		close(task.done)
		return nil
...
func (t worker) DeployTo(ctx context.Context, peer *agent.Peer) error {
	task := &pending{Peer: peer, timeout: t.timeout, done: make(chan error)}
...
func (t Deploy) Deploy(c cluster) (int64, bool) {
	...
	if failure := (polling{}).Await(ctx, initial, t.dispatcher, c, t.worker.check); failure != nil {
